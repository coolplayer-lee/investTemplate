...
> **2026-09-05-r5执行修订（估值修复与决策反馈）**：价值发现/价值回归可用全球同商业模式与国内同市场比较论证估值修复；区分相对折价、合理价值与主观情景，补齐买入、仓位、退出和反馈记录。不要求消除所有不确定性才买入，也不把单只股票的倍数推广为通用阈值。详见[第六章](template/06-估值与安全边际.md#rerating-analysis)、[第七章](template/07-决策流程与持仓管理.md#decision-feedback)及[报告契约](template/00-报告输出契约.md)。
